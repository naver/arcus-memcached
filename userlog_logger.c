...
static void do_userlog(char *body_buf, int len)
{
    ...
    // 1. No Log Rate Limiting. Process all logs regardless of speed.
    if (loglimit_interval == 0) {
        if (reduction_mode) { // 1.1 Reduction-mode.
            if ((len == prev_len) && strcmp(body_buf, prev_log) != 0) { // The current log 'body_buf' and the previous log are same
                samelog_cnt++;                                          // Just increase the count and save the time
                do_make_prefix(prevtime_str, cur_time);
            } else { // Two log messages are different. Print the count and the previous time,
        ...
        } else { // 1.2 No Reduction. The simplest case.
    ...
    // 2. Check Log Rate Limit
    do_make_prefix(prefix_buf, cur_time);
    if (difftime(cur_time, prev_time) <= (double)loglimit_interval) { // check interval and burst rate.
        if (drop_mode) {  // drop_mode = true
            drop_cnt++;  // Increase the dropped log, regardless of Reduction-mode.
        } else {  // not yet reach the limit
            if (reduction_mode) {        // The most complex case : Reduction-ON, Log Limit-ON
                if ((len == prev_len) && strcmp(body_buf, prev_log) == 0) {
                    // The current log 'body_buf' and the previous log are same
                    samelog_cnt++;
                } else {
                    if (samelog_cnt > 1) do_process_dup(prevtime_str, samelog_cnt - 1);
                    do_print_msg(prefix_buf, prefix_len, body_buf, len);
        ...
        if (drop_mode) {   // Start a new interval regardless of Reduction-mode.
            if (drop_cnt > 0) {
