...
ARCUS 서버를 운영 및 모니터링하기 위해 아래 명령들을 제공한다.
...
ARCUS Cache Server에 새로 맺어지는 연결에 대해 인증 과정을 요구할지 설정/조회한다.
기본 값은 서버 구동 시 `-S` 옵션 지정한 경우 on, 그렇지 않은 경우 off이다.

```
config auth [on]\r\n
```

<a id="command-logging"></a>
## Command Logging 명령
...
3. 정해진 대기 시간이 지나면, 남아있는 클라이언트 연결과 무관하게 프로세스를 종료한다.

```
shutdown [<seconds>]\r\n
```
- `<seconds>` - 프로세스 종료 전 대기 시간을 초(second) 단위로 설정한다.
  대기 시간은 ZooKeeper 연결 종료 등의 종료 준비가 끝난 시점부터 계산한다.
  - 생략하면, 2초로 설정한다.
...
- 이미 shutdown 요청을 수신하여 종료 대기 중인 상태에서 새로운 shutdown 명령을 수신하면,
그 요청은 거부되고 `DENIED` 응답을 반환한다. 즉, 종료 시간은 최초 요청을 기준으로 유지되며 재조정할 수 없다.
- 종료 대기 중에 SIGTERM 또는 SIGINT 시그널을 수신하면, 남은 대기 시간과 무관하게 곧바로 종료를 진행한다.
...
- `reload auth`: SASL 인증 기능이 사용 중일 때만 사용 가능, ZooKeeper에 저장된 최신 정보 기준으로 인증/권한 정보 재캐싱
...
| "CLIENT_ERROR bad command line format" | protocol syntax 틀림 |
| "ERROR unknown command" | reload 기능이 없는 캐시 서버 |
