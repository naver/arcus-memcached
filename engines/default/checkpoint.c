...
typedef struct _chkpt_st {
    ...
    bool     pause;       /* checkpoint module pause */
    ...
    int      completetime; /* latest snapshot completed time */
...
/* Delete all backup files except the most recently completed snapshot. */
...
static int do_chkpt_create_files(chkpt_st *cs)
{
    ...
    if (fd < 0) {
        ...
        if (errno == ENOMEM) {
            logger->log(EXTENSION_LOG_WARNING, NULL,
                    "Deactivate the checkpoint module until it clears all garbage files.\n");
            cs->pause = true;
        }
        return -1;
    }
...
static void* chkpt_thread_main(void* arg)
{
    ...
    while (engine->initialized) {
        if (ret == CHKPT_SUCCESS) {
            elapsed_time += do_chkpt_sleep(cs, CHKPT_DEFAULT_SLEEP);
        } else {
            elapsed_time += do_chkpt_sleep(cs, CHKPT_ERROR_SLEEP);
        }
        ...
        if (need_remove) {
            ...
            if (cs->pause && !need_remove) {
                cs->pause = false;
                logger->log(EXTENSION_LOG_INFO, NULL,
                            "The file removal work is complete. checkpoint module reactivate.\n");
            }
        }

        if (!cs->pause && elapsed_time >= cs->interval) {
            ret = do_checkpoint(cs);
            if (ret != CHKPT_SUCCESS) {
                logger->log(EXTENSION_LOG_WARNING, NULL, "Failed in checkpoint\n");
                if (ret == CHKPT_ERROR_FILE_REMOVE || cs->pause) {
                    need_remove = true;
                }
            }
            /* snapshot has completed. reset elapsed_time */
            if (ret == CHKPT_SUCCESS || ret == CHKPT_ERROR_FILE_REMOVE) {
                elapsed_time = 0;
            }
        }
    }
