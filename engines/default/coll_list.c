...
#ifdef ENABLE_LARGE_ITEM
        if (is_large) {
            large_pa *pa = (large_pa *)((char *)item_get_key(it)
...
static void do_list_elem_unlink(list_meta_info *info, list_elem_item *elem,
                                bool is_large, enum elem_delete_cause cause)
#else
static void do_list_elem_unlink(list_meta_info *info, list_elem_item *elem,
...
#ifdef ENABLE_LARGE_ITEM
        if (info->stotal > 0 && !is_large) { /* apply memory space */
#else
...
static uint32_t do_list_elem_delete(list_meta_info *info, const int index,
                                    const uint32_t count, bool is_large,
                                    enum elem_delete_cause cause)
#else
...
#endif
{
    ...
    while (elem != NULL) {
        ...
        fcnt++;
#ifdef ENABLE_LARGE_ITEM
        do_list_elem_unlink(info, elem, is_large, cause);
#else
